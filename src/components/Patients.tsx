...
export const Patients: React.FC<PatientsProps> = ({users = []}) => {
  return (
    <div className="bg-white w-96 rounded-2xl flex flex-col mt-8 overflow-hidden">
      ...
      <ul className="overflow-y-auto flex-1 my-5">
        {users.map(user => (
          <li className={classNames("hover:bg-[#D8FCF7]", {'bg-[#D8FCF7]': user.name === 'Jessica Taylor'})}>
            <a href="/" className="flex justify-between px-5 py-4">
              <div className="flex gap-3">
                <img src={user.profile_picture} alt={user.name} className="h-12 w-12" />
